...
interface RoomPosition {
  ...
  findClosestByPath<T extends Structure>(
    ...
    opts?: FindPathOpts &
      FilterOptions<FIND_STRUCTURES> & { algorithm?: string }
  ): T | null;
  /**
   * Find the object with the shortest path from the given position. Uses A* search algorithm and Dijkstra's algorithm.
...
interface Store {
  /**
   * Returns capacity of this store for the specified resource, or total
   * capacity if resource is undefined.
   */
  ...
  /**
   * Returns the capacity used by the specified resource, or total used
   * capacity for general purpose stores if resource is undefined.
   */
